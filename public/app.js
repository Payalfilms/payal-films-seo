...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const payValidationHint = document.getElementById('payValidationHint');
  const changeScreenshotBtn = document.getElementById('changeScreenshotBtn');
...
&lt;/script&gt;
  ...
  // Real-time validation for mandatory WhatsApp proof submission
  function updateWhatsAppButtonState() {
    if (!sendWhatsAppProofBtn) return false;

    const nameVal = customerNameInput ? customerNameInput.value.trim() : '';
    const rawPhone = customerPhoneInput ? customerPhoneInput.value.trim() : '';
    const digitsOnlyPhone = rawPhone.replace(/\D/g, '');
    const hasScreenshot = Boolean(selectedScreenshotFile);

    const missing = [];
    if (nameVal.length < 2) missing.push('Full Name');
    if (digitsOnlyPhone.length < 10) missing.push('10-digit WhatsApp No.');
    if (!hasScreenshot) missing.push('Payment Screenshot');

    const isValid = missing.length === 0;

    if (isValid) {
      sendWhatsAppProofBtn.disabled = false;
      sendWhatsAppProofBtn.classList.remove('btn-disabled');
      sendWhatsAppProofBtn.removeAttribute('title');
      if (payValidationHint) {
        payValidationHint.className = 'pay-validation-hint valid';
        payValidationHint.innerHTML = '<span>✅ Sabhi required details fill ho gayi hain! Ab aap <strong>"Send Screenshot on WhatsApp"</strong> par click kar sakte hain.</span>';
      }
    } else {
      sendWhatsAppProofBtn.disabled = true;
      sendWhatsAppProofBtn.classList.add('btn-disabled');
      sendWhatsAppProofBtn.setAttribute('title', `Baki hai: ${missing.join(', ')}`);
      if (payValidationHint) {
        payValidationHint.className = 'pay-validation-hint';
        payValidationHint.innerHTML = `<span>⚠️ Baki hai: <strong>${missing.join('</strong>, <strong>')}</strong>. Sab bharne ke baad hi WhatsApp button enable hoga.</span>`;
      }
    }
  ...
  // Attach live input listeners for validation
  if (customerNameInput) customerNameInput.addEventListener('input', updateWhatsAppButtonState);
  if (customerPhoneInput) customerPhoneInput.addEventListener('input', updateWhatsAppButtonState);

  // Change / Remove Screenshot Button
  if (changeScreenshotBtn) {
    changeScreenshotBtn.addEventListener('click', (e) => {
      e.stopPropagation();
      selectedScreenshotFile = null;
      if (paymentScreenshotInput) paymentScreenshotInput.value = '';
      if (screenshotPreviewWrap) screenshotPreviewWrap.style.display = 'none';
  ...
  if (screenshotDropzone && paymentScreenshotInput) {
    screenshotDropzone.addEventListener('click', (e) => {
      if (e.target !== paymentScreenshotInput && e.target !== changeScreenshotBtn) {
  ...
  if (sendWhatsAppProofBtn) {
    sendWhatsAppProofBtn.addEventListener('click', async (e) => {
      ...
      if (!updateWhatsAppButtonState()) {
        showToast('Kripya Full Name, 10-digit WhatsApp number aur Screenshot upload karein.', 'error');
        return;
      }
